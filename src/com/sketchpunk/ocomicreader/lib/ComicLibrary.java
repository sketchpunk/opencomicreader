...
public class ComicLibrary {
	...
	public static void removeComic(Context context, String id, boolean delComic) {
		...
		if (delComic) { // Delete comic from device
			String comicPath = db.scalar(
					"SELECT path FROM ComicLibrary WHERE comicID = '"
							+ id.replace("'", "''") + "';", null);
		...
		if (db.delete("ComicLibrary", "comicID = '" + id.replace("'", "''")
				+ "'", null) > 0) {
			file = new File(Environment.getExternalStorageDirectory()
					.getAbsolutePath()
					+ "/OpenComicReader/thumbs/"
					+ id
					+ ".jpg");
			if (file.exists())
				file.delete();
	...
	public static void setComicProgress(Context context, String id, int state,
			boolean applySeries) {
		String sql = "UPDATE ComicLibrary SET ";
		sql += (state == 0) ? "pgRead=0,pgCurrent=0"
				: "pgRead=pgCount,pgCurrent=pgCount"; // 0-Reset or 1-Mark as
														// Read.

		if (applySeries)
			sql += " WHERE series in (SELECT series FROM ComicLibrary WHERE comicID = '"
					+ id + "')";
		else
			sql += " WHERE comicID = '" + id + "'";
	 ...
	 * Manage Covers
	 */
	public static boolean createThumb(int coverHeight, int coverQuality,
			iComicArchive archive, String coverPath, String saveTo) {
		boolean rtn = false;
		...
		if (iStream != null) {
			...
			try {
				...
				// calc scale
				int scale = (bmpOption.outHeight > coverHeight) ? Math
						.round((float) bmpOption.outHeight / coverHeight) : 0;
				bmpOption.inSampleSize = scale;
				bmpOption.inJustDecodeBounds = false;

				// Load bitmap and re scale
				iStream.close(); // the first read should of closed the stream.
									// Just do it just in case it didn't
	...
	public static void clearCovers(Context context) {
		...
		sage.data.Sqlite.execSql(context,
				"UPDATE ComicLibrary SET isCoverExists=0", null);
	}

	/*
	 * Manage Series
	 */
	public static void setSeriesName(Context context, String comicID,
			String seriesName) {
		ContentValues cv = new ContentValues();
		cv.put("series", seriesName);
		sage.data.Sqlite.update(context, "ComicLibrary", cv, "comicID=?",
				new String[] { comicID });
	}

	public static void renameSeries(Context context, String oldSeries,
			String newSeries) {
		ContentValues cv = new ContentValues();
		cv.put("series", newSeries);
		sage.data.Sqlite.update(context, "ComicLibrary", cv, "series=?",
				new String[] { oldSeries });
	}
	...
	protected static class ThumbFindFilter implements java.io.FileFilter {
		public boolean accept(File o) {
			if (o.isDirectory())
				return false;
			else if (o.getName().toLowerCase(Locale.getDefault())
					.endsWith(".jpg"))
				return true;
			return false;
		}
