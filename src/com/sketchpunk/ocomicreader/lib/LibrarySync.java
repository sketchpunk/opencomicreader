...
public class LibrarySync implements Runnable {
	...
	// When crawling for comic files, include folders that have images in them.
	private boolean mIncImgFlds = false;
	// Skip finding new comics, just process the current library.
	private boolean mSkipCrawl = false;
	// User can choose to force series name from the parent folder name instead
	// of using the series parser.
	private boolean mUseFldForSeries = false;
	...
	public int crawlComicFolders() {
		...
		if (!mSyncFld1.isEmpty() && !mSyncFld2.isEmpty()) {
			...
			sWhere = MediaStore.Images.Media.DATA + " like ? OR "
					+ MediaStore.Images.Media.DATA + " like ?) GROUP BY (2";
		...
		Cursor eCur = activity.getContentResolver().query(
				MediaStore.Images.Media.EXTERNAL_CONTENT_URI, cols, sWhere,
				sWhereAry, null); // "MAX(datetaken) DESC"
		Cursor iCur = activity.getContentResolver().query(
				MediaStore.Images.Media.INTERNAL_CONTENT_URI, cols, sWhere,
				sWhereAry, null);
		...
		if (mcur.moveToFirst()) {
			...
			SQLiteStatement sql = mDb
					.compileStatement("INSERT INTO ComicLibrary(isCoverExists,pgCount,pgRead,pgCurrent,comicID,title,path,series) VALUES(0,0,0,1,?,?,?,?);");
			mDb.beginTransaction();

			do {
				// Check if already in library
				// Remove Filename from the path
				path = sage.io.Path.removeLast(mcur.getString(dCol));
				sendProgress(path);

				tmp = mDb.scalar(
						"SELECT comicID FROM ComicLibrary WHERE path = '"
								+ path.replace("'", "''") + "'", null);
				if (!tmp.isEmpty())
					continue;
	...
	public int crawlComicFiles() {
		...
		while (!stack.isEmpty()) {
			...
			for (File file : fList) {
				if (file.isDirectory())
					// add to stack to continue to crawl.
	...
	private void processLibrary() {
		// Can't delete records with an open cursor, save IDs and delete later.
		StringBuilder delList = new StringBuilder();
		// Page Count, Path to Cover Entry, Path to Meta Data
		String[] comicInfo = { "", "", "" };
		...
		Cursor cur = mDb
				.raw("SELECT comicID,path,isCoverExists,series,title FROM ComicLibrary",
						null);
		...
		for (boolean isOk = cur.moveToFirst(); isOk; isOk = cur.moveToNext()) {
			...
			if (cur.getString(2).equals("0")) {
				...
				sql = "pgCount=" + comicInfo[0];
				// Create ThumbNail
				if (ComicLibrary.createThumb(mCoverHeight, mCoverQuality,
						archive, comicInfo[1], mCachePath + comicID + ".jpg"))
					sql += ",isCoverExists=1";
				...
				if (comicMeta != null) {
					if (!comicMeta[0].isEmpty())
						sql += ",title = '"
								+ comicMeta[0].replaceAll("'", "''") + "'";
					if (!comicMeta[1].isEmpty())
						sql += ",series = '"
								+ comicMeta[1].replaceAll("'", "''") + "'";
				}

				// Save information to the db.
				mDb.execSql(String.format(
						"UPDATE ComicLibrary SET %s WHERE comicID = '%s'", sql,
						comicID), null);
				if (comicMeta != null && comicMeta[1] != "")
					// Since series was updated from meta, Don't continue the
					// rest of the loop which handles the series
					continue;
			}
			...
			if (seriesName == null
					|| seriesName.isEmpty()
					|| seriesName
							.compareToIgnoreCase(ComicLibrary.UKNOWN_SERIES) == 0) {
				if (mUseFldForSeries)
					...
					// if seriesName ends up being the path, use the parent
					// folder as the series name.
		...
		if (delList.length() > 0) {
			...
			mDb.execSql(String.format(
					"DELETE FROM ComicLibrary WHERE comicID in (%s)", delList
