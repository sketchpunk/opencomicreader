...
public class ViewActivity extends Activity implements ComicPageView.CallBack,
		ComicLoader.CallBack, DialogInterface.OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (prefs.getBoolean("keepScreenOn", true))
			winFlags |= WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON;

		// Apply Flags
		if (winFlags != 0)
			getWindow().addFlags(winFlags);
		...
		if (uri != null) {
			...
			currentPage = Math
					.max(Integer.parseInt(dbData.get("pgCurrent")), 1);
		}// if
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		case R.id.mnu_scaleh:
			mImageView.setScaleMode(ComicPageView.ScaleToHeight);
			break;
		case R.id.mnu_scalew:
			mImageView.setScaleMode(ComicPageView.ScaleToWidth);
			break;
		case R.id.mnu_scalen:
			mImageView.setScaleMode(ComicPageView.ScaleNone);
			break;
		case R.id.mnu_goto:
			sage.ui.Dialogs.NumPicker(this, "Go to Page", 1,
	...
	public void onComicPageGesture(int gestureID) {
		...
		if (!this.mPref_FlingEnabled
				&& (gestureID == ComicPageView.FlingRight || gestureID == ComicPageView.FlingLeft))
			return;
